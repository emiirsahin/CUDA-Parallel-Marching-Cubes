    ...
    float3 *meshNormals)
{
    ...
    int totalWork = NumX * NumY * NumZ;

    float3 *intersect = new float3[12];

    for (int i = threadId; i < totalWork; i += blockDim.x * gridDim.x)
    {
        int ix = i / (NumY * NumZ);
        int iy = (i - (ix * NumY * NumZ)) / NumZ;
        int iz = (i - (ix * NumY * NumZ) - (iy * NumZ));
    ...
    float3 *meshNormals)
{
    ...
    int totalWork = NumX * NumY * NumZ * frameNum;

    float3 *intersect = new float3[12];

    for (int i = threadId; i < totalWork; i += blockDim.x * gridDim.x)
    {
        int fnum = i / (NumY * NumZ * NumX);
        int ix = (i - (fnum * NumY * NumZ * NumX)) / (NumY * NumZ);
        int iy = (i - (fnum * NumY * NumZ * NumX) - (ix * NumY * NumZ)) / NumZ;
        int iz = (i - (fnum * NumY * NumZ * NumX) - (ix * NumY * NumZ) - (iy * NumZ));
        float curTwist = 1.0 * fnum * maxTwist / frameNum;

        if (fnum < frameNum && ix < NumX && iy < NumY && iz < NumZ)
        {
            float x = domainP->min.x + ix * cubeSizeP->x;
            float y = domainP->min.y + iy * cubeSizeP->y;
            float z = domainP->min.z + iz * cubeSizeP->z;
            ...
            for (int i = 0; i < 8; ++i)
            {
                float sd = opTwist(pos[i], curTwist);
                if (sd == 0)
                    sd += 1e-6;
                sdf[i] = sd;
            }
            ...
            // now create and store the triangle data
            int offset = (NumX * NumY * NumZ * fnum + NumZ * NumY * ix + NumZ * iy + iz) * 16;
            Triangulate(curTwist, meshVertices + offset, meshNormals + offset, signConfig, intersect);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    cudaMalloc((void **)&domain_d, sizeof(Rect3));
    cudaMalloc((void **)&cubeSize_d, sizeof(float3));
    ...
    cudaMalloc((void **)&meshVertices_d, frameSize * sizeof(float3));
    cudaMalloc((void **)&meshNormals_d, frameSize * sizeof(float3));
    ...
    checkCudaErrors(cudaMallocHost(&meshVertices_h, frameSize * sizeof(float3)));
    float3 *meshNormals_h;
    checkCudaErrors(cudaMallocHost(&meshNormals_h, frameSize * sizeof(float3)));
    ...
    float3 *meshVertices_h3;
    checkCudaErrors(cudaMallocHost(&meshVertices_h3, frameSize * frameNum * sizeof(float3)));
    float3 *meshNormals_h3;
    checkCudaErrors(cudaMallocHost(&meshNormals_h3, frameSize * frameNum * sizeof(float3)));

    Rect3 *domain_d3;
    float3 *cubeSize_d3;
    float3 *meshVertices_d3;
    float3 *meshNormals_d3;

    cudaMalloc((void **)&domain_d3, sizeof(Rect3));
    cudaMalloc((void **)&cubeSize_d3, sizeof(float3));

    cudaMemcpy(domain_d3, &domain, sizeof(Rect3), cudaMemcpyHostToDevice);
    cudaMemcpy(cubeSize_d3, &cubeSize, sizeof(float3), cudaMemcpyHostToDevice);

    cudaMalloc((void **)&meshVertices_d3, frameSize * frameNum * sizeof(float3));
    cudaMalloc((void **)&meshNormals_d3, frameSize * frameNum * sizeof(float3));
    ...
    if (part == -1 || part == 3)
    {
        ...
        //                     Launch the kernel                     //
        ///////////////////////////////////////////////////////////////
        MarchCubeCUDAMultiframe<<<numBlocks, numThreads>>>(domain_d3, cubeSize_d3, frameNum, maxTwist, 0, meshVertices_d3, meshNormals_d3);
        checkCudaErrors(cudaDeviceSynchronize());
        end = high_resolution_clock::now();
        kernelTime = (duration<double>(end - start)).count();

        start = high_resolution_clock::now();
        cudaMemcpy(meshVertices_h3, meshVertices_d3, frameSize * frameNum * sizeof(float3), cudaMemcpyDeviceToHost);
        cudaMemcpy(meshNormals_h3, meshNormals_d3, frameSize * frameNum * sizeof(float3), cudaMemcpyDeviceToHost);
        ///////////////////////////////////////////////////////////////
        //              Copy the result back to host                 //
        ///////////////////////////////////////////////////////////////
        end = high_resolution_clock::now();
        ...
        for (frame = 0; frame < frameNum; frame++)
        {
            if (saveObj)
            {
                ...
                WriteObjFile(frameSize, meshVertices_h3 + offset, meshNormals_h3 + offset, filename);
            }

            if (correctTest)
            {
                TestCorrectness(frameSize, meshVertices_h3 + offset, meshNormals_h3 + offset, frame);
            }
            offset += frameSize;
        }
        end = high_resolution_clock::now();
        ...
        printf("\nPart3\nTime taken: \nKernels: %f sec | Performance: %f mln_cubes/sec \nMemcpy:  "
               ...
               memcpyTime, extraTime, (kernelTime + memcpyTime + extraTime));
        showMemUsage();
        cudaMemset(meshNormals_h3, 0, frameNum * frameSize * sizeof(float3));
        cudaMemset(meshVertices_h3, 0, frameNum * frameSize * sizeof(float3));
        twist = 0;
    }

    cudaFree(domain_d3);
    cudaFree(cubeSize_d3);
    cudaFree(meshVertices_d3);
    cudaFree(meshNormals_d3);
    cudaFree(meshVertices_h3);
    cudaFree(meshNormals_h3);

    float3 *meshVertices_h4;
    checkCudaErrors(cudaMallocHost(&meshVertices_h4, frameSize * sizeof(float3)));
    float3 *meshNormals_h4;
    checkCudaErrors(cudaMallocHost(&meshNormals_h4, frameSize * sizeof(float3)));

    Rect3 *domain_d4;
    float3 *cubeSize_d4;
    float3 *meshVertices_d4[2];
    float3 *meshNormals_d4[2];
    ...
    cudaMalloc((void **)&domain_d4, sizeof(Rect3));
    cudaMalloc((void **)&cubeSize_d4, sizeof(float3));
    cudaMalloc((void **)&meshVertices_d4[0], frameSize * sizeof(float3));
    cudaMalloc((void **)&meshNormals_d4[0], frameSize * sizeof(float3));
    cudaMalloc((void **)&meshVertices_d4[1], frameSize * sizeof(float3));
    cudaMalloc((void **)&meshNormals_d4[1], frameSize * sizeof(float3));

    cudaMemcpy(domain_d4, &domain, sizeof(Rect3), cudaMemcpyHostToDevice);
    cudaMemcpy(cubeSize_d4, &cubeSize, sizeof(float3), cudaMemcpyHostToDevice);
    ...
    {
        start = high_resolution_clock::now();
        for (frame = 0; frame < frameNum + 1; frame++)
        {
            if (frame < frameNum)
            {
                MarchCubeCUDATwoPointers<<<numBlocks, numThreads, 0, streamCompute>>>(domain_d4, cubeSize_d4, twist, 0, meshVertices_d4[(frame) % 2], meshNormals_d4[(frame) % 2]);
            }

            if (frame > 0)
            {
                cudaMemset(meshNormals_h4, 0, frameSize * sizeof(float3));
                cudaMemset(meshVertices_h4, 0, frameSize * sizeof(float3));
                cudaMemcpyAsync(meshVertices_h4, meshVertices_d4[(frame - 1) % 2], frameSize * sizeof(float3), cudaMemcpyDeviceToHost, streamCopy);
                cudaMemcpyAsync(meshNormals_h4, meshNormals_d4[(frame - 1) % 2], frameSize * sizeof(float3), cudaMemcpyDeviceToHost, streamCopy);
                ///////////////////////////////////////////////////////////
                //                   Launch the kernel                   //
                ///////////////////////////////////////////////////////////
                ...
                if (saveObj)
                {
                    ...
                    WriteObjFile(frameSize, meshVertices_h4, meshNormals_h4, filename);
                }
                ...
                {
                    checkCudaErrors(cudaDeviceSynchronize());
                    TestCorrectness(frameSize, meshVertices_h4, meshNormals_h4, frame);
                }
    ...
    cudaFree(domain_d4);
    cudaFree(cubeSize_d4);
    cudaFree(meshVertices_d4);
    cudaFree(meshNormals_d4);
    cudaFree(meshVertices_h4);
    cudaFree(meshNormals_h4);
    ///////////////////////////////////////////////////////////////////
    //                         Free memory                           //
    ///////////////////////////////////////////////////////////////////
}
